...
RepoViewer = {
  ...
  showRef: function(remoteAndRefName) {
    ...
    origin.fetchHistory(ref.sha, 10, function() {
      var commit = RepoViewer.repo.getObject(ref.sha)
      RepoViewer.displayCommitAndParents(commit, 10)
      console.log("display tree: " + commit.tree)
    ...
    if ($("#commit-" + commit.sha).length == 0) {
      ...
      if (max == 0) {
        ...
        _(commit.parents).each(function(parentSha) {
          var parent = RepoViewer.repo.getObject(parentSha)
          if (parent) {
            RepoViewer.displayCommitAndParents(RepoViewer.repo.getObject(parentSha), max - 1)
          }
        })
      }
    }
