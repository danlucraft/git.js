...
RepoViewer = {
  ...
  TreeDiffGenerator: function(prevTree, thisTree, callback) {
    this.thisTree = thisTree
    ...
    this.compareBlobEntry = function(prevBlobEntry, thisBlobEntry) {
      if(!prevBlobEntry) {
        ...
        changeInfo += "  <div class='filename'>new: " + thisBlobEntry.name + "</div>"
        changeInfo += "  <hr>"
        changeInfo += "</div>"
        ...
        var changeInfo = ""
        changeInfo += "<div class='changed-file'>"
        changeInfo += "  <div class='filename'>changed: " + thisBlobEntry.name + "</div>"
        changeInfo += "  <hr>"
        changeInfo += "</div>"
        this.changes.push(changeInfo)
      }
    ...
    RepoViewer.repo.object(commit.tree, function(thisTree) {
      RepoViewer.repo.object(commit.parents[0], function(parent) {
        RepoViewer.repo.object(parent.tree, function(prevTree) {
          var treeDiffGenerator = new RepoViewer.TreeDiffGenerator(prevTree, thisTree, function(changes) {
            
          })
          $("#diff-view-window").append(treeDiffGenerator.generate())
        })
      })
    })
  },
