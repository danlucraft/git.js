...
Pack.prototype.matchOffsetDeltaObject = function(header, ready) {
  ...
  var self = this
    , ofs = self.offset
  inflate(utils.bytesToString(deflated), function(err, uncompressedData, compressedLength) {
...
Pack.prototype.matchNonDeltaObject = function(header, ready) {
  var data = this.rest()
  var deflated = utils.stripZlibHeader(data)
    , self = this
    , ofs = self.offset
...
Pack.prototype.expandOffsetDeltas = function(ready) {
  ...
  process.stderr.write(total + 'objects')
  _(this.objects).each(function(object) {
