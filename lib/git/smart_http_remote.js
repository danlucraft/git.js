...
var SmartHttpRemote = function(repo, name, repoUrl) {
  ...
  this.fetchRef = function(wantRef, callback) {
    ...
    var body = SmartHttpRemote.refWantRequest(wantRef, repo.haveRefs(repo.getAllRefs()))
    var thisRemote = this
    $.ajax({
      url: url,
      data: body,
      ...
      success: function(data, textStatus, xhr) {
        ...
        if (!objectDatas) {
          throw("Upload pack contained no objects! Data is: " + utils.stringToBytes(binaryData).toString())
        }
        var i, object
        var newObjects = []
      ...
      error: function(xhr, data, e) {
        utils.displayError("ERROR Status: " + xhr.status + ", response: " + xhr.responseText)
      }
    });
  }
