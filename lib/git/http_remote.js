  ...
  if (this.packs) {
    ...
    _(_(this.packs).keys()).each(function(packSha) {
      ...
      if (packInfo.index && packInfo.pack) {
        ...
        if (offset) {
          packInfo.pack.getObjectAtOffset(offset, function(err, thisObject) {
            ready(err, thisObject)
          })
        }
      }
    })
...
HttpRemote.parseObjectData = function(sha, compressedData, ready) {
  ...
  inflate(utils.bytesToString(deflated), function(err, data) {
    var offset = 0
    ...
    ready(null, Objects.make(sha, type, utils.bytesToString(rest())))
  });
}
