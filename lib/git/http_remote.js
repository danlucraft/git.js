  ...
  this.fetchObjectLoose = function(sha, callback) {
    ...
    $.ajax({
      ...
      success: function(data) {
        callback(HttpRemote.parseObjectData(sha, data))
      },
      error: function(xhr) {
        callback(null)
      }
...
HttpRemote.parseObjectData = function(sha, compressedData) {
  var deflated = utils.stripZlibHeader(utils.stringToBytes(compressedData))
  var data = new BinaryFile(inflate(utils.bytesToString(deflated)).toString())
  var offset = 0
  ...
  var type = utils.bytesToString(peek(3))
  advance(3)
  if (type === "com") {
    type = "commit"
    advance(4)
  ...
  return Objects.make(sha, type, utils.bytesToString(rest()))
}
