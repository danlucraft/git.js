...
JsGit.objects = {
  ...
  Commit: function(sha, data) {
    ...
    this.timestamp = /(\d+) \+\d\d\d\d/.exec(this.author)[1]
    this.dateCommitted = new Date(parseInt(this.committer.split(">")[1].split(" ")[1])*1000)
    this.dateAuthored = new Date(parseInt(this.author.split(">")[1].split(" ")[1])*1000)
