...
var fixturePackFile = function() {
  var arr = eval(fs.readFileSync(__dirname + "/fixtures/pack.json").toString('binary'))
...
exports.PackFileParser = {
  "parses out objects": function(test) {
    ...
    packFile.parseAll(function(err, data) {
  ...
  "supports random access by offset": function(test) {
    var packFile = new Pack(fixturePackFile())
    packFile.getObjectAtOffset(10229, function(err, data) {
      test.deepEqual(
        data.id(), 
        { type: 'tree'
  ...
  "supports random access of delta objects by offset": function(test) {
    var packFile = new Pack(fixturePackFile())
    packFile.getObjectAtOffset(11633, function(err, object) {
      if (err) throw err;
